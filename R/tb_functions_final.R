...
# set up region + age code: one region/state at a time
setup_region_data = function(data_file_path){
  
  data = read.csv(data_file_path, stringsAsFactors = FALSE)
  
  # get data into regions
  reg_1 = c('CT', 'ME', 'MA', 'NH', 'RI', 'VT')
  reg_2 = c('NJ', 'NY')
  reg_3 = c('DE', 'DC', 'MD', 'PA', 'VA', 'WV')
  ...
  for(i in 1:nrow(data)){
    if(data$state[i] %in% reg_1){
      region[i] = 'reg_1'
    } else if(data$state[i] %in% reg_2){
      region[i] = 'reg_2'
    } else if(data$state[i] %in% reg_3){
      region[i] = 'reg_3'
    } else if(data$state[i] %in% reg_4){
      region[i] = 'reg_4'
    } else if(data$state[i] %in% reg_5){
      region[i] = 'reg_5'
    } else if(data$state[i] %in% reg_6){
      region[i] = 'reg_6'
    } else if(data$state[i] %in% reg_7){
      region[i] = 'reg_7'
    } else if(data$state[i] %in% reg_8){
      region[i] = 'reg_8'
    } else if(data$state[i] %in% reg_9){
      region[i] = 'reg_9'
    } else if(data$state[i] %in% reg_10){
      region[i] = 'reg_10'
    } else if(data$state[i] %in% reg_AK){
      region[i] = 'reg_AK'
    } else if(data$state[i] %in% reg_HI){
      region[i] = 'reg_HI'
    }
  }
  
  data$region = unlist(region)
...
setup_import = function(data){
  ...
  import_sum = dat_g %>% filter(arrivecat == '<2 years') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total) 
...
setup_usborn = function(data){
  ...
  import_sum = dat_g %>% filter(arrivecat == '<2 years') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, usborn) 
...
setup_age = function(data){
  ...
  import_sum = dat_g %>% filter(arrivecat == '<2 years') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, age) 
...
# overall us with age, FOR USBORN ONLY (NO IMPORTATION)
setup_age_no_importation = function(data){
  
  dat_g = data %>% group_by(date, age)
  
  case_sum = dat_g %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, age)
  extra_sum = dat_g %>% filter(extra == 'EXTRAPULM ONLY') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, age) 
  import_sum = rep(0, nrow(case_sum))
  
  dat = data.frame(case_sum$date, case_sum$age, case_sum$cases_total, extra_sum$cases_total, import_sum)
  colnames(dat) = c('date', 'age', 'cases', 'extra', 'imports')
  
  return(dat)
}

# region without importation - need total cases, extrapulmonary cases
# don't actually need this function
setup_reg = function(data){
  
  # group data by date and region - want to look at each timepoint and location individually
  dat_g = data %>% group_by(date, region)
  
  # sum of all cases
  case_sum = dat_g %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region)
  # sum of extrapulmonary cases
  extra_sum = dat_g %>% filter(extra == 'EXTRAPULM ONLY') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region) 
  
  dat = data.frame(case_sum$date, case_sum$region, case_sum$cases_total, extra_sum$cases_total)
  colnames(dat) = c('date', 'region', 'cases', 'extra')
  
  return(dat)
}


# region with importation - need total cases, extra cases, import cases
setup_reg_import = function(data){
  
  dat_g = data %>% group_by(date, region)
  
  case_sum = dat_g %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region)
  extra_sum = dat_g %>% filter(extra == 'EXTRAPULM ONLY') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region) 
  import_sum = dat_g %>% filter(arrivecat == '<2 years') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region) 

  dat = data.frame(case_sum$date, case_sum$region, case_sum$cases_total, extra_sum$cases_total, import_sum$cases_total)
  colnames(dat) = c('date', 'region', 'cases', 'extra', 'imports')
  
  return(dat)
}


setup_reg_usborn = function(data){
  
  dat_g = data %>% group_by(date, region, usborn)
  
  case_sum = dat_g %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region, usborn)
  extra_sum = dat_g %>% filter(extra == 'EXTRAPULM ONLY') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region, usborn) 
  import_sum = dat_g %>% filter(arrivecat == '<2 years') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region, usborn) 
  
  dat = data.frame(case_sum$date, case_sum$region, case_sum$usborn, case_sum$cases_total, extra_sum$cases_total, import_sum$cases_total)
  colnames(dat) = c('date', 'region', 'usborn', 'cases', 'extra', 'imports')
  
  return(dat)
}


setup_reg_age = function(data){
  
  dat_g = data %>% group_by(date, region, age)
  
  case_sum = dat_g %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region, age)
  extra_sum = dat_g %>% filter(extra == 'EXTRAPULM ONLY') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region, age) 
  import_sum = dat_g %>% filter(arrivecat == '<2 years') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region, age) 
  
  if(length(import_sum$cases_total) == 0){
    dat = data.frame(case_sum$date, case_sum$region, case_sum$age, case_sum$cases_total, extra_sum$cases_total,
                     rep(0, dim(case_sum)[1]))
  } else{
    dat = data.frame(case_sum$date, case_sum$region, case_sum$age, case_sum$cases_total, extra_sum$cases_total,
                     import_sum$cases_total)
  }
  
  colnames(dat) = c('date', 'region', 'age', 'cases', 'extra', 'imports')
  
  return(dat)
}

setup_reg_age_usborn = function(data){
  
  dat_g = data %>% group_by(date, region, age, usborn)
  
  case_sum = dat_g %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region, age, usborn)
  extra_sum = dat_g %>% filter(extra == 'EXTRAPULM ONLY') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region, age, usborn) 
  import_sum = dat_g %>% filter(arrivecat == '<2 years') %>% summarize(cases_total = sum(cases)) %>% ungroup() %>% select(date, cases_total, region, age, usborn) 
  
  dat = data.frame(case_sum$date, case_sum$region, case_sum$age, case_sum$cases_total, extra_sum$cases_total, import_sum$cases_total)
  colnames(dat) = c('date', 'region', 'age', 'usborn', 'cases', 'extra', 'imports')
...
# estimate_option - specify which estimation function to use
  ...
  if (est_option == 'region'){
    for(i in 1:1000){
      rt = find_rts_strata(samples[[i]], num_strata = 10, int_mat = diag(10))
      rt_df[i] = rt$rts
    }
  }
  
  else if (est_option == 'region_import'){
    for(i in 1:1000){
      rt = find_rts_complete(samples[[i]], num_strata = 10, int_mat = diag(10))
      rt_df[i] = rt$rts
    }
  }
  
  else if (est_option == 'region_age'){
    for(i in 1:1000){
      rt = find_rts_strata_2(samples[[i]], num_strata = 10, int_mat = diag(10),
                             num_strata_2 = 7, int_mat_2 = diag(7))
      rt_df[i] = rt$rts
    }
  }
  
  else if (est_option == 'region_age_import'){
    for(i in 1:1000){
      rt = find_rts_complete_2(samples[[i]], num_strata = 10, int_mat = diag(10),
                             num_strata_2 = 7, int_mat_2 = diag(7))
      rt_df[i] = rt$rts
    }
  }
  
  else if (est_option == 'region_age_2'){
    for(i in 1:1000){
      rt = find_rts_strata_2(samples[[i]], num_strata = 10, int_mat = diag(10),
  ...
  else if (est_option == 'region_age_import_2'){
    for(i in 1:1000){
      rt = find_rts_complete_2(samples[[i]], num_strata = 10, int_mat = diag(10),
                               num_strata_2 = 7, int_mat_2 = age_matrix)
      rt_df[i] = rt$rts
  ...
  else if (est_option == 'all_usborn'){
    for(i in 1:1000){
      rt = find_rts_strata(samples[[i]], num_strata = 3, int_mat = diag(3))
      rt_df[i] = rt$rts
    }
  }
  ...
  else if (est_option == 'all_age'){
    for(i in 1:1000){
      rt = find_rts_strata(samples[[i]], num_strata = 7, int_mat = diag(7))
      rt_df[i] = rt$rts
    }
  }
  ...
  else if (est_option == 'all_age_2'){
    for(i in 1:1000){
      rt = find_rts_strata(samples[[i]], num_strata = 7, int_mat = age_matrix)
      rt_df[i] = rt$rts
    }
  }
